...
async def _(event: MessageEvent, args=CommandArg()):
    if isinstance(event, GroupMessageEvent) and event.group_id == 878697241:
        await m_copywrite.finish("你是？")
    if event.sender.user_id == 1754958648:
        await m_copywrite.finish("你是？")
...
async def _(event: MessageEvent, args=CommandArg()):
    if isinstance(event, GroupMessageEvent) and event.group_id == 878697241:
        await m_new_meaning.finish("你是？")
    if event.sender.user_id in {1754958648, 3123077383}:
        await m_new_meaning.finish("你是？")
    args = args.extract_plain_text().strip()
